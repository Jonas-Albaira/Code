...
@Component({
  ...
  template: `
    ...
    <dialog #dlg>
      <form method="dialog" class="modal" (ngSubmit)="submit()">
...
export class TasksPageComponent implements OnInit {
  ...
  form: Partial<Task> = { title: '', description: '', status: 'pending' };
  private _editing = signal<Task | null>(null);
  ...
  submit() {
    ...
    if (!editing || !editing.id) {
      this.api.create(this.form).subscribe({
        next: () => { this.close(); this.refresh(); }
      });
    } else {
      this.api.update(editing.id, this.form).subscribe({
        next: () => { this.close(); this.refresh(); }
      });
    }
  }
